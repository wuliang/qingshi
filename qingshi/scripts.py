...
class ResetCommand(Command):
    ...
    def finalize_options(self):
        if not self.category:
            print 'No category is set, assume all categories.'
        if self.category:
            self.category = self.category.split(',')
    ...
    def run(self):
        ...
        cfg = _loadConfig()
        seg_service = service.SegumentService(cfg)
        seg_service.db.clean(self.category)
        print 'Done.'
...
class RefineCommand(Command):
    description = 'refine lexicon database to generate relations'
    user_options = [
        ('category=', 'c', 'category name'),
    ]
    ...
    def finalize_options(self):
        if not self.category:
            raise DistutilsOptionError('Must set category to refine')
...
class DumpCommand(Command):
    ...
    user_options = [
        ...
        ('encoding=', 'e', 'encoding of text file'),
        ('category=', 'c', 'category name'),
        ('level=', 'l', 'level name: detail, brief'),
    ]
    ...
    def run(self):
        ...
        if not c:
            print 'Category %s not exist' % self.category
            return
        c.dump(self.text_file, self.level)
